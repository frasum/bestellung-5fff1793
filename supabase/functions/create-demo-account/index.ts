...
serve(async (req) => {
  ...
  try {
    ...
    if (userExists) {
      return new Response(
        JSON.stringify({ error: 'An account with this email already exists. Please login or use a different email.' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // Generate a random password (user won't need it, but it's required)
    ...
    // Get the organization that was auto-created by the trigger
    ...
    if (!profile?.organization_id) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to create demo organization' }),
