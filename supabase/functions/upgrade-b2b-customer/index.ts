...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { type = 'customer', email, password, organization_name, subscription_tier } = body;
    ...
    if (type === 'supplier') {
      return await handleSupplierUpgrade(supabaseAdmin, body);
    } else {
      return await handleCustomerUpgrade(supabaseAdmin, body);
    }
  } catch (error) {
...
async function handleCustomerUpgrade(supabaseAdmin: any, body: any) {
  const { customer_id, email, password, organization_name, subscription_tier } = body;
  ...
  const { userId, organizationId } = await createUserAndOrganization(
    supabaseAdmin,
    email,
    password,
    organization_name,
  ...
  console.log("Customer upgrade completed successfully");
...
async function handleSupplierUpgrade(supabaseAdmin: any, body: any) {
  const { account_id, email, password, organization_name, subscription_tier } = body;

  if (!account_id || !email || !password || !organization_name) {
    return new Response(
      JSON.stringify({ error: "Missing required fields" }),
      { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  ...
  // Check if account exists and is not already upgraded
  const { data: account, error: accountError } = await supabaseAdmin
    .from("supplier_b2b_accounts")
    .select("*, b2b_supplier_vendors(*), b2b_supplier_vendor_articles(*)")
    .eq("id", account_id)
    .single();
  ...
  if (account.upgraded_organization_id) {
    return new Response(
      JSON.stringify({ error: "Account has already upgraded" }),
      { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
  ...
  const { userId, organizationId } = await createUserAndOrganization(
    ...
    organization_name,
    subscription_tier,
    'b2b_supplier_upgrade',
    null
  );

  // Migrate vendors to suppliers
  const vendors = account.b2b_supplier_vendors || [];
  const vendorIdMap = new Map<string, string>();
  ...
  for (const vendor of vendors) {
    ...
    vendorIdMap.set(vendor.id, newSupplier.id);
    console.log(`Migrated supplier vendor ${vendor.name} -> supplier ${newSupplier.id}`);
  }

  // Migrate articles
  const articles = account.b2b_supplier_vendor_articles || [];
  let migratedArticles = 0;
  ...
  // Update account with upgrade info
  await supabaseAdmin
    .from("supplier_b2b_accounts")
    .update({
      upgraded_organization_id: organizationId,
  ...
  console.log("Supplier upgrade completed successfully");
