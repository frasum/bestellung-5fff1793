            ...
            <div key={item.label} className="space-y-2">
                  ...
                  {!isUnlimited && (
                    <p className="text-xs text-muted-foreground">
                      {item.remaining === 'unlimited' 
                        ? t('subscription.unlimited', 'Unbegrenzt')
                        : t('subscription.remaining', '{{count}} verbleibend', { count: item.remaining })}
                    </p>
                  )}
                </div>
              </div>
              {!isUnlimited && (
                <div className="relative">
                  ...
                  <div 
                    className={`absolute top-0 left-0 h-full rounded-full transition-all ${getProgressColor(item.percentage)}`}
