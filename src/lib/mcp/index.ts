...
export default defineMcp({
  ...
  version: "0.1.0",
  instructions:
    "Tools to inspect a Bestellung.pro account: list suppliers, search articles, list recent orders, and fetch a single order with its line items. All calls run as the signed-in user; data is scoped to that user's organization via row-level security.",
  auth: auth.oauth.issuer({
    issuer: `https://${projectRef}.supabase.co/auth/v1`,
    acceptedAudiences: "authenticated",
  }),
  tools: [listSuppliersTool, listOrdersTool, searchArticlesTool, getOrderTool],
});
